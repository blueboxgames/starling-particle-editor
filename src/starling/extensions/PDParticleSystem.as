...
package starling.extensions
{
    ...
    public class PDParticleSystem extends ParticleSystem
    {
		...
		private function parseJsonConfig(config:Object):void 
		{
            ...
            setProperty("startSize", config.startSize);
            setProperty("startSizeVariance", config.startSizeVariance);
            setProperty("endSize", config.endSize);
            setProperty("endSizeVariance", config.endSizeVariance);
            setProperty("emitAngle", deg2rad(config.emitAngle));
            setProperty("emitAngleVariance", deg2rad(config.emitAngleVariance));
            setProperty("startRotation", deg2rad(config.startRotation));
            setProperty("startRotationVariance", deg2rad(config.startRotationVariance));
            setProperty("endRotation", deg2rad(config.endRotation));
            setProperty("endRotation", deg2rad(config.endRotation));
            setProperty("endRotationVariance", deg2rad(config.endRotationVariance));
            ...
            setProperty("radialAccelerationVariance", config.radialAccelerationVariance);
            setProperty("tangentialAcceleration", config.tangentialAcceleration);
            setProperty("tangentialAccelerationVariance", config.tangentialAccelerationVariance);
